...
% avr = handles.Value.points(:,1000);
% avr = mean(handles.Value.curve,2);            % curve矩阵是
axes(handles.axes1)
hold off
handles.h1 = plot(avr);
axes(handles.axes2)
...
if str2num(get(handles.l_filter_edit,'string')) > 1
    l_filter = str2num(get(handles.l_filter_edit,'string'));
elseif str2num(get(handles.l_filter_edit,'string')) <= 1
