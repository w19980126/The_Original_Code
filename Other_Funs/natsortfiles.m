...
%% Input Wrangling %%
% 好鸡肋的一段代码，看了这么久
assert(iscell(X),'First input <X> must be a cell array.')       % 保证X必须是cell，否则报错
tmp = cellfun('isclass',X,'char') & cellfun('size',X,1)<2 & cellfun('ndims',X)<3;      
% 对cell里面每个子元素进行逻辑运算，确认cell内部每一个元素是一个char、一行、且二维，其实对于一般的文件名，都符合这个要求
assert(all(tmp(:)),'First input <X> must be a cell array of strings (1xN character).')      % 合者返回，不合报错
%
